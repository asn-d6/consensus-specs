...
| - | - | - |
| `WHISK_SHUFFLING_SET_SIZE`         | `uint64(2**14)`  (= 16,384) | size of shuffling (i.e. candidate) set                                |
| `WHISK_PROPOSERS_SET_SIZE`         | `uint64(2**13)`  (= 8,192)  | size of proposer set                                                  |
...
Invariant: The protocol should produce enough proposers to last for an entire shuffling phase: `WHISK_PROPOSERS_SET_SIZE = WHISK_SHUFFLE_DURATION * SLOTS_PER_EPOCH`
...
| Name | Value | Description |
| - | - | - |
| `WHISK_TRIVIAL_PERMUTATION_X` | `TODO{Depends on CRS of shuffle proof}` | x coordinate of commitment to trivial permutation                           |
| `WHISK_TRIVIAL_PERMUTATION_Y` | `TODO{Depends on CRS of shuffle proof}` | y coordinate of commitment to trivial permutation                           |
| `WHISK_TRIVIAL_PERMUTATION`   | `BLSG1PointFromAffine(WHISK_TRIVIAL_PERMUTATION_X, WHISK_TRIVIAL_PERMUTATION_Y)` | commitment to trivial permutation  |
...
class Validator(Container):
    ...
    whisk_com_k: BLSG1Point

class BeaconState(Container):
    # ...
    whisk_shuffling: Vector[WhiskTracker, WHISK_SHUFFLING_SET_SIZE]  # [New in Whisk]
    whisk_proposers: Vector[WhiskTracker, WHISK_PROPOSERS_SET_SIZE]  # [New in Whisk]
    # ...
...
def whisk_candidate_selection(state: BeaconState, epoch: Epoch) -> None:
    ...
    for i in range(WHISK_SHUFFLING_SET_SIZE):
        ...
        state.whisk_shuffling[i] = state.validators[index].whisk_tracker


def whisk_proposer_selection(state: BeaconState, epoch: Epoch) -> None:
    """
    Select proposers from the shuffling set
    """
    ...
    for i in range(WHISK_PROPOSERS_SET_SIZE):
        index = compute_shuffled_index(uint64(i), uint64(len(state.whisk_shuffling)), seed)
        state.whisk_proposers[i] = state.whisk_shuffling[index]
...
def whisk_verify_proposer(state: BeaconState, block: BeaconBlock) -> None:
    ...
    tracker = state.whisk_proposers[state.slot % WHISK_PROPOSERS_SET_SIZE]

    assert bls.IsValidDLEQProof(block.whisk_opening_proof,
                                tracker.k_r_G, tracker.r_G,
                                proposer.whisk_com_k, BLS_G1_GENERATOR)
...
class BeaconBlockBody(Container):
    ...
    whisk_tracker: WhiskTracker  # [New in Whisk]
    whisk_com_k: BLSG1Point  # [New in Whisk]


def feistel_encrypt(index, r):
    """
    Apply `r` Feistel rounds on `index`.
    """
    def F(x):  # Permutation sbox acting as the F function of the Feistel cipher
        return (14,61,111,46,72,70,25,120,64,52,62,104,45,53,44,0,68,69,79,100,78,42,58,76,3,102,29,81,22,122,74,23,11,98,32,4,86,9,10,2,57,1,35,31,34,127,109,96,113,37,8,21,20,93,67,116,83,107,105,41,63,60,82,103,39,24,123,26,50,12,16,6,87,124,97,13,38,27,91,65,49,121,90,88,89,28,114,59,71,48,33,106,51,94,47,54,75,126,84,118,73,80,115,92,56,40,125,108,101,30,18,5,36,17,117,19,112,77,95,119,7,99,55,15,85,66,43,110)[x]
...
def whisk_process_shuffled_trackers(state: BeaconState, permutation_commitment: BLSG1Point,
    ...
    pre_shuffle_trackers = [state.whisk_shuffling[i] for i in shuffle_indices]
    ...
    for i, shuffle_index in enumerate(shuffle_indices):
        state.whisk_shuffling[shuffle_index] = post_shuffle_trackers[i]


def commitment_is_unique(state: BeaconState, com_k: BLSG1Point) -> bool:
    # Check that no validator has `com_k` as their commitment
    for validator in state.validators:
        if validator.whisk_com_k == com_k:
            return False
...
def register_commitments_from_block(state: BeaconState, block: BeaconBlock, proposer: Validator):
    ...
    assert commitment_is_unique(state, block.body.whisk_commitment)
    ...
    assert bls.IsValidDLEQProof(block.body.whisk_registration_proof,
                                ...
                                block.body.whisk_com_k, BLS_G1_GENERATOR)

    # Everything is OK: register the commitments
    proposer.whisk_com_k = block.body.whisk_com_k
...
def process_whisk_block(state: BeaconState, block: BeaconBlock) -> None:
    ...
    permutation_commitment = proposer.whisk_tracker.r_G
    whisk_process_shuffled_trackers(state, permutation_commitment,
                                   block.body.whisk_shuffled_trackers, block.body.whisk_shuffle_proof)

    # Allow proposer to register fresh WHISK commitments once
    if proposer.whisk_tracker.r_G == WHISK_TRIVIAL_PERMUTATION:
        assert proposer.whisk_tracker.r_G != WHISK_TRIVIAL_PERMUTATION
        ...
        assert proposer.whisk_registration_proof.T_1 == Bytes48()
        assert proposer.whisk_registration_proof.T_2 == Bytes48()
        assert proposer.whisk_registration_proof.s_1 == Bytes48()
        assert proposer.whisk_registration_proof.s_2 == Bytes48()
        assert proposer.whisk_tracker.r_G == Bytes48()
        assert proposer.whisk_tracker.k_r_G == Bytes48()
        assert proposer.whisk_com_k == Bytes48()
...
```python
def get_unique_commitment(state: BeaconState, index: ValidatorIndex):
    # Use try-and-increment to find a unique predictable `k` for this validator
    i = 0
    while True:
        hashed_counter = hash(validator_index + uint_to_bytes(i))

        k = BLSFrScalar(hashed_counter)
        com_k = bls.ScalarMult(k, BLS_G1_GENERATOR)

        # Return this commitment if it's unique
        if commitment_is_unique(state, com_k):
            return k, com_k

        i += 1


def whisk_get_initial_commitments(state: BeaconState, index: ValidatorIndex):
    # Create trivial k and com(k)
    k, com_k = get_unique_commitment(state, index)

    # Create trivial tracker (r*G, k*r*G)
    tracker = WhiskTracker(
        r_G=WHISK_TRIVIAL_PERMUTATION,
        k_r_G=bls.ScalarMult(k, WHISK_TRIVIAL_PERMUTATION),
    )

    return com_k, tracker


def get_validator_from_deposit(state: BeaconState, deposit: Deposit) -> Validator:
    com_k, tracker = whisk_get_initial_commitments(state, len(state.validators))  # [New in Whisk]
    ...
    return Validator(
        ...
        whisk_com_k=com_k,  # [New in Whisk]
