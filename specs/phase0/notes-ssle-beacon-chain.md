...
```python
# bad bad bad breaks uniqueness property
class DLEQProof:
    ...
    #    - k is the dlog of `k_r_G = k*(r_G)` [SSLE tracker]
    #    - k is also the dlog of `k*G` [K commitment]
...
class BeaconBlock(Container):
    ...
    # Commitment K=k*G for this opening
...
def ssle_verify_proposer(state: BeaconState, block: BeaconBlock) -> None:
    # First verify that the included K commitment corresponds to this proposer
    proposer = state.validators[block.proposer_index]
    assert proposer.ssle_commitment == block.ssle_commitment

    # Now verify the opening proof
...
def register_commitments_from_block(state: BeaconState, block: BeaconBlock, proposer: Validator):
    ...
    # Ensure uniqueness of K commitment
    for validator in state.validators:
        assert validator.ssle_commitment != block.body.ssle_commitment

    # Check that the same k is used in the tracker and the K commitment
...
def process_ssle_block(state: BeaconState, block: BeaconBlock) -> None:
    ...
    ssle_process_shuffled_trackers(state, proposer.ssle_tracker.r_G,
